...
CRedisPublisher::CRedisPublisher():_event_base(0),
	_event_thread(0),
	_redis_context(0)
{
}
...
//初始化 - libevent and event信号
bool CRedisPublisher::init()
{
	//initialize the event
	_event_base = evnet_base_new();	//创建libevent对象
	if(NULL == _event_base)
	{
	...
	int ret = sem_init(&_evnet_sem, 0, 0);
	if(ret != 0)
	{
...
bool CRedisPublisher::connect()
{
	...
	//attach the event
	//将事件绑定到 redis context上， 使设置给redis的回调跟事件关联
	redisLibeventAttach(_redis_context, _event_base);

	//创建事件处理线程
	int ret = pthread_create(&_event_thread, 0, &CRedisPublisher (Hidden::event_thread, this);
	...
	redisAsyncSetConnectCallback(_redis_context, &CRedisPublisher::disconnect_callback);
...
{
	int ret = redisAsyncCommand(_redis_context, &CRedisPublisher::command_callback,	this, "PUBLISH %s %s",
