...
bool CRedisSubscriber::subscribe(const std::string &channel_name)
{
	int ret = redisAsyncCommand(_redis_context,
			&CRedisSubscriber::command_callback, this,
			"SUBSCRIBE %s", channel_name.c_str());
...
void CRedisSubscriber::command_callback(redisAsyncContext *redis_context, void *reply,
		void *privdata)
{
	...
	if(redis_reply->type == REDIS_REPLY_ARRAY && redis_reply->elements == 3)
	{
		printf(": Recieve message:%s:%d:%s:%d:%s:%d\n",
				redis_reply->element[0]->str, (int)redis_reply->element[0]->len,
				redis_reply->element[1]->str, (int)redis_reply->element[1]->len,
				redis_reply->element[2]->str, (int)redis_reply->element[2]->len);
